...
our $input_suffix='';		# 数据文件后缀（clean或unmapped）
...
while(<IN>){
	...
	process_cmd("cp ./$contig_type/$sample.$contig_type.fa $sample_dir/contig_sequences.fa");
	...
	&process_cmd("$BIN_DIR/blast_parse_table22.pl $sample.blastn.paired $sample.blastn.table1");	# parse blast results
	&process_cmd("$BIN_DIR/blast_filter3.pl $sample.blastn.table1 $sample.blastn.table 0.75 5");	# get paired query and hit according to e value
	system("rm $sample.blastn.paired");								# remove temp file
	system("rm $sample.blastn.table1");								# remove temp file
	
	# separate known and novel contig
	&process_cmd("$BIN_DIR/query_filter1.pl $sample.blastn.table ./$contig_type/$sample.$contig_type.fa $sample.novel.contigs 60 50 > $sample.known.contigs");
	...
	#再得到average depth信息
	my $sample_reads= $sample;	#read文件，需要align到新contigs上
...
# system("touch virus_identify.$contig_type.finished");

system("rm -r aligned");
system("rm -r assembled");
