...
while(<IN>){
	...
	&process_cmd("$BIN_DIR/samtools index $sample.sorted.bam");#建立索引文件，用于IGV人工查看
	...
	&process_cmd("cut -f1 $sample.known.identified > known.references.list");#取出hit name（第1列）,将用于生成sam文件
	...
	&process_cmd("$BIN_DIR/arrange_col2.pl $sample.known.identified1 bowtie.log > $sample.known.identified");#重新生成$sample.known.identified（格式有变化）
	...
	system("mv $sample.sorted.bam $sample_dir/known.sorted.bam");#这个保留，用于IGV人工查看
	system("mv $sample.sorted.bam.bai $sample_dir/known.sorted.bam.bai");#这个保留，用于IGV人工查看
	...
	if($contig_type ne "aligned")#如果需要进一步检查novel的病毒
	{
		...
		&process_cmd("$BIN_DIR/arrange_col2.pl $sample.novel.identified1 bowtie.log > $sample.novel.identified");#重新生成$sample.novel.identified（格式有变化）
...
close(IN);
system("rm hit_virus.list");#这个文件名用了两次，最后一起删除
system("rm hit_virus.fa");#这个文件名用了两次，最后一起删除
system("rm hit_virus.fa.fai");#这个文件名用了两次，最后一起删除
system("rm remainding.fa");#这个文件名用了两次，最后一起删除
system("rm *.tem");
system("rm *.log");
