...
#########################################################################################
# bowtie2pileup.pl --file_list <FILE> --reference [FILE]  --out_type <String> --mismatchs [INT]
...
main: {
    ...
    while (<IN>) {
		...
		if($mismatchs){
			&process_cmd("$BIN_DIR/bowtie $index_name -v $mismatchs -p $thread_num -q $sample.clean -S -a --best $sample.pre.sam") unless (-s "$sample.pre.sam");		

		}
		else{
			&process_cmd("$BIN_DIR/bowtie $index_name -n $dist_seed -l $len_seed -e $quality_sum -p $thread_num -q $sample.clean -S -a --best $sample.pre.sam") unless (-s "$sample.pre.sam");	
		}
